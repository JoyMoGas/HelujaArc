document.addEventListener('DOMContentLoaded', () => {
    ...
    const handleScroll = () => {
        if (window.scrollY > 100) { // Ajusta el valor según el punto en el que quieras cambiar la posición
            ...
            logo.style.top = '10px'; // Ajusta según el espacio necesario en la navbar
            logo.style.left = '20px'; // Ajusta según el espacio necesario en la navbar
            logo.style.height = '60px'; // Ajustado para coincidir con el tamaño en la navbar
            logo.style.transform = 'translateX(0) scale(0.6)'; // Ajusta el tamaño si es necesario
        } else {
